...
class ToolSimulationRequest(BaseModel):
    """Optional request body for a direct MCP execution test."""

    # In real integrations, callers often pass an existing correlation ID.
    # If omitted, the API generates one for the execution.
...
class AgentChatRequest(BaseModel):
    ...
    user_message: str
    correlation_id: str | None = None
    # False means planning only. True lets chat run MCP execution when the
...
class ToolSimulationResponse(BaseModel):
    """Public API shape for MCP execution results."""

    # In mock mode, low/medium-risk tools return sample data.
    # In remote mode, they execute through the managed MCP server.
...
class ApprovalRequestResponse(BaseModel):
    ...
    # The API shape can stay the same when approvals move from the current
    # local store to a workflow or database.
...
class AgentChatResponse(BaseModel):
    """Response body for the safe agent chat endpoint."""

    # This is the main frontend response. It supports plan-only, completed
    # execution, missing data, and approval-required branches.
    correlation_id: str
    status: str
    message: str
    tool_called: bool
    ...
    # Populated only when chat was explicitly allowed to execute and policy
